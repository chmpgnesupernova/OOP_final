...
T x, y, mean;
public:
    E(T init_x = 0, T init_y = 0) : x(init_x), y(init_y) { mean = (x + y)/2; }
    void Print() const { std::cout << x << ' ' << y << ' ' << mean << std::endl; }
};
...
char x, y, mean;
public:
    E(char init_x = 0, char init_y = 0) : x(init_x), y(init_y) { mean = (x + y)/2; }
    void Print() const { std::cout << static_cast<int>(x) << ' ' << static_cast<int>(y) << ' ' << static_cast<int>(mean) << std::endl; }
};

template<typename T> 
void Fn1(T it1, T it2) {
    for (; it1 != it2; ++it1)
    std::cout << *it1 << ' ';
    std::cout << std::endl;
}
...
void Fn2 (std::vector<int>& v) {
    v.erase( std::remove_if( v.begin(), v.end(), [v](int t){ return t < 0; } ), v.end() );
}
