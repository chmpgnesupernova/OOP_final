...
int x;
public:
	Super (int xx = 0) : x(xx) { std::cout << "Super" << std::endl;}
...
int y;
public:
	Sub (int xx = 0, int yy = 0) : Super(xx), y(yy){ std::cout << "Sub" << std::endl;}
	void Print() const { 
		Super::Print();
		std::cout << ", " << y; }
	virtual ~Sub(){std::cout << "~Sub"<< std::endl;}
};
...
public:
	Data (T xx = 0) : p (std::make_shared<T>(xx)) {}
	T Get() {
		return *p;
	}
	void Set(T yy) {
